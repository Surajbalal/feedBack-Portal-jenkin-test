...
try {
   ...
   const admin = await Admin.findById(verifyUser._id); 
        console.log(admin)
...
router.post('/register', async (req, res) => {
    try {
        const { adminId, email, name, password, roleType } = req.body;
        ...
        return res.status(200).json({
            ...
            admin: { adminId: admin.adminId, name: admin.name, email: admin.email, roleType: admin.roleType },
            token
        });
